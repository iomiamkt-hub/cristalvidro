...
function FieldInput({
  ...
  return (
    <div className="border-b border-white/[0.07] pb-1 mb-1">
      <label className="block text-[10px] text-zinc-600 uppercase tracking-widest mb-2">{label}</label>
      <input
        type={type}
        value={value}
        ...
        className={cn(
          "w-full bg-transparent text-[15px] text-white placeholder:text-zinc-700",
...
export function StepContact() {
  ...
  if (submitted) {
    return (
      ...
      >
        ...
        >
          <svg className="w-6 h-6 text-zinc-900" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
          </svg>
        </motion.div>

        <div className="space-y-2">
          <h2 className="text-2xl font-semibold text-white">Enviado</h2>
          <p className="text-zinc-600 text-sm leading-relaxed">
            Responderemos em até 2 horas úteis pelo WhatsApp.
          </p>
        </div>

        <div className="w-full text-left border-t border-white/[0.06] pt-6 space-y-1">
          <p className="text-xs text-zinc-700 uppercase tracking-widest">Estimativa</p>
          <p className="text-3xl font-bold text-white tabular-nums">{formatCurrency(result.total)}</p>
        </div>
        ...
        <div className="w-full flex flex-col gap-2.5 mt-2">
          <a
            ...
            className="w-full h-12 rounded-xl bg-[#25D366] text-white text-sm font-semibold flex items-center justify-center gap-2 hover:bg-[#1db954] transition-colors"
          >
          ...
          <button
            onClick={reset}
            className="w-full h-10 text-xs text-zinc-700 hover:text-zinc-500 transition-colors"
          >
            Novo orçamento
          </button>
        </div>
  ...
  return (
    <div className="flex flex-col flex-1">
      <h1 className="text-[1.75rem] font-semibold text-white tracking-tight leading-[1.15] mb-2">
        Quase lá
      </h1>
